...
contract MartianMarket is ERC721Full, Ownable {
    ...
    modifier landRegistered(uint token_id) {
        require(_exists(token_id), "Land not registered yet. Please contact front office.");
        _;
    }
    ...
    function pendingReturn(uint token_id, address) public view landRegistered(token_id) returns(uint) {
        MartianAuction auction = auctions[token_id];
